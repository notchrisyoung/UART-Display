...
void InfoScreen::processUartData(JsonDocument& doc) {
...
void InfoScreen::sendUartData(const String& message) {
    // Send data over UART if needed
    // Add logic to send data over UART
}
void InfoScreen::setUARTcallback(void (*callback)(const String& message)) {
    // Set the UART callback to send data from this screen
}
void InfoScreen::highlight(int item) {
...
void InfoScreen::clearHighlight() {
...
void InfoScreen::upPress() {
    if (selectedIndex > 0) {
        selectedIndex--;
        highlight(selectedIndex + 1);
    }
}
void InfoScreen::downPress() {
    if (selectedIndex < menuItemCount - 1) {
        selectedIndex++;
        highlight(selectedIndex + 1);
    }
}
void InfoScreen::leftPress() {
    // Handle left button press if needed
}
void InfoScreen::rightPress() {
    // Handle right button press if needed
}
void InfoScreen::selectPress() {
